...
public class BaseAuthenticatedTest {
    ...
    private void createPermissions() {
        List<String[]> permissionData = List.of(
                ...
                new String[] { PermissionEnum.SYNC_ROLE_WITH_PERMISSIONS.toString(), "sync role with permissions" });

        List<Permission> permissions = permissionData
                .stream()
                .map(data -> new Permission(data[0],
                        data[1]))
    ...
    private void syncAdminPermissions() {
        ...
        Set<Permission> adminPermissions = new HashSet<>(permissionJpaRepository.findByNameIn(adminPermissionNames));
    ...
    private void createAdminUser() {
        User user = new User();
        Role role = roleJpaRepository
                .findByName(RoleEnum.ADMIN.toString()).get();
    ...
    protected void generateAuthorizationToken() throws IllegalArgumentException, UnsupportedEncodingException {
